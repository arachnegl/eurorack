# Rings - Compiling & Flashing Guide

This document describes how to compile the firmware for [Rings](https://github.com/arachnegl/eurorack/tree/master/rings) and flash it onto
...
2. **Define Compiler Path**: Define the `TOOLCHAIN_PATH` variable pointing to the compiler binary directory inside the
   [makefile](https://github.com/arachnegl/eurorack/blob/master/rings/makefile#L48):
   ```makefile
...
Compilation commands are run from the project root or the `rings` directory:

### A. Build standard binary
Generates build output binaries inside the `build/rings` folder:
```bash
make
```

### B. Generate Audio Firmware Update WAV File
Rings can be flashed by playing a modulated audio file into its audio input. To build this WAV file:
```bash
make wav
```
...
python stm_audio_bootloader/qpsk/encoder.py \
    ...
    build/rings/rings.bin
```
It outputs `rings.wav`, which uses QPSK (Quadrature Phase-Shift Keying) modulation to transmit data.
